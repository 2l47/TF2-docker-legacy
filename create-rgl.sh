...
sudo docker run -d --net=host -v $(pwd)/tf2-data_rgl:/home/steam/tf-dedicated/ --name=tf2-dedicated_rgl -e SRCDS_TOKEN=`cat srcds_token.txt` -e SRCDS_PORT=27015 -e SRCDS_PW="`cat server_password_rgl.txt`" -e SRCDS_MAXPLAYERS=16 -e SRCDS_STARTMAP="koth_product_rcx" -e SRCDS_TICKRATE=100 cm2network/tf2:sourcemod
